...
func (l *List[T]) Head() *Element[T] {
	defer adt.With(adt.Lock(l.root().mtx()))
	r := l.root()
	if r.Next() == nil {
		return r
	}
...
func (l *List[T]) root() *Element[T]         { return l.head.Do(l.init) }
...
func LocksHeld(mtxs []*sync.Mutex) []*sync.Mutex {
	erc.InvariantOk(mtxs != nil, "must hold the correct set of locks for the operation")
	...
	erc.InvariantOk(numNils == 0, "cannot lock with missing locks:", numNils)

	locked := make([]*sync.Mutex, len(mtxs))

	defer func() {
		if p := recover(); p != nil {
			defer WithAll(locked)
			panic(p)
